treeJSON = d3.json('usertreeretrieve.php', function(error, treeData){
	...
	//define the zoomListener which calls the zoom function on the "zoom" even constrained within the scaleExtents
	...
	dragListener = d3.behavior.drag().on("dragstart", function(d){
		...
		//it's important that we suppress the mouseover even on the node being dragged. Otherwise it will absorb the mouseover even and the underlying node will not detect it d3.select(this).attr('pointer-events', 'none');
	}).on("drag", function(d){
		if(d == root){
			return;
		}
	...
	function click(d){
		...
		console.log(d);
		d = toggleChildren(d);
		//console.log(d.firstname + ' ' + d.name);
		update(d);
		centerNode(d);
	}
	
	function expandinfo(d){
		if(d3.event.defaultPrevented) return; //click suppressed
		var id = d.name,
	...
	function update(source){
		...
		}).style("fill-opacity", 0).on('click', expandinfo);
		...
		node.select('text').attr("x", function(d){
			...
			if(d.name != 'MTI'){
				var info = d.firstname + ' ' + d.lastname + ' (' + d.name + ')';
				if(d.status == 'I'){
					info += ' - Inactive';
