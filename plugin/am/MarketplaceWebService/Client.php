...
class MarketplaceWebService_Client implements MarketplaceWebService_Interface
{
  ...
  private  $config = array ('ServiceURL' => null,
                            'UserAgent' => 'PHP Client Library/2010-03-31 (Language=PHP5)',
                            'SignatureVersion' => 2,
  ...
  const MWS_CLIENT_VERSION = '2010-03-31';
   ...
   */
  private function invoke(array $parameters, $dataHandle = null, $contentMd5 = null)
  {
    ...
    if (empty($this->config['ServiceURL'])) {
      ...
      do {
        try {
          $response = $this->performRequest($actionName, $parameters, $dataHandle, $contentMd5);
   ...
   * @return array
   */
  private function performRequest($action, array $parameters, $dataHandle = null, $contentMd5 = null) {

    $curlOptions = $this->configureCurlOptions($action, $parameters, $dataHandle, $contentMd5);
   ...
   * @return array
   */
  private function configureCurlOptions($action, array $parameters, $streamHandle = null, $contentMd5 = null) {
    ...
    if ($requestType == RequestType::POST_UPLOAD) {
      ...
      $serviceUrl .= '?' . $this->getParametersAsString($parameters);
      ...
      $curlOptions[CURLOPT_POSTFIELDS] = $this->getParametersAsString($parameters);
  ...
  private function addRequiredParameters(array $parameters)
  {
    ...
    $parameters['Timestamp'] = $this->getFormattedTimestamp(new DateTime('now'));
